...
```sh
make docker-build docker-push IMG=<some-registry>/k8s-partition-workload:tag
```
...
```sh
make deploy IMG=<some-registry>/k8s-partition-workload:tag
```
...
```sh
make build-installer IMG=<some-registry>/k8s-partition-workload:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/k8s-partition-workload/<tag or branch>/dist/install.yaml
```
