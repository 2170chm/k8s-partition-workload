...
func TestCreateApplyRevision(t *testing.T) {
	...
	revision, err := NewRevision(pw, 1, pw.Status.CollisionCount)
	if err != nil {
		t.Fatal(err)
	}
	...
	restoredpw, err := ApplyRevision(pw, revision)
	if err != nil {
		t.Fatal(err)
	}
	restoredRevision, err := NewRevision(restoredpw, 2, restoredpw.Status.CollisionCount)
	if err != nil {
		t.Fatal(err)
	}
...
func TestApplyRevision(t *testing.T) {
	...
	currentRevision, err := NewRevision(pw, 1, pw.Status.CollisionCount)
	if err != nil {
		t.Fatal(err)
	}
	...
	updateRevision, err := NewRevision(pw, 2, pw.Status.CollisionCount)
	if err != nil {
		t.Fatal(err)
	}

	restoredCurrentpw, err := ApplyRevision(pw, currentRevision)
	if err != nil {
		t.Fatal(err)
	}
	...
	restoredUpdatepw, err := ApplyRevision(pw, updateRevision)
	if err != nil {
		t.Fatal(err)
	}
